...
import { UserService, User, Profile } from 'src/app/core';
  ...
  category$: Observable<Category>;
	...
	ngOnInit() {
		...
		this.route.data.subscribe((data: { listing: Listing }) => {
			...
			this.category$ = this.categoryQuery.selectEntity(this.selectedListing.category_id);

			this.userService.getProfile(this.selectedListing.user_id).subscribe((res) => {
	...
	changeDate() {
		...
		if (this.paymentRateTitle === 'daily') {
			this.f.end_date.setValue(this.addDays(this.f.start_date.value, this.f.quantity.value));
			return true;
			console.log('daily');
		}
		if (this.paymentRateTitle === 'weekly') {
			this.f.end_date.setValue(this.addWeeks(this.f.start_date.value, this.f.quantity.value));
			return true;
		}
		if (this.paymentRateTitle === 'monthly') {
			this.f.end_date.setValue(this.addMonths(this.f.start_date.value, this.f.quantity.value));
			return true;
		} else {
	...
	updateBookingForm(data) {
		const totalServiceFee = this.serviceFee * (data.quantity * data.price);
		const total = data.quantity * data.price + totalServiceFee;
	...
	addDays(start, quantity: number) {
		let endDate = new Date(start);
		endDate.setDate(endDate.getDate() + quantity);
		return endDate;
	}

	addWeeks(start, quantity) {
	...
	addMonths(start, quantity) {
		let endDate = new Date(start);
		//endDate.setDate(endDate.getDate() + (quantity * 7))
		let newDate = new Date(endDate.setMonth(endDate.getMonth() + quantity));
		return newDate;
	}
