...
export class UserService {
	...
	isAdmin(): Observable<boolean> {
		return this.http.get<boolean>(API_ENDPOINT + '/auth/isadmin').pipe(
			map(
				(res) => {
					console.log(res);
					return res;
	...
	update(id, user): Observable<any> {
		return this.http.post(API_ENDPOINT + '/auth/userinfo/' + id, user).pipe(
			map((data) => {
	...
	changePassword(id, data): Observable<any> {
		return this.http.post(API_ENDPOINT + '/auth/password/' + id, data).pipe(
			map((data) => {
				// Update the currentUser observable
				// this.currentUserSubject.next(data);
				return data;
